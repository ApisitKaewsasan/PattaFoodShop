			...
			switch ($data['type']) {
				...
				case "owl_banner":
					...
					if(isset($data['setting']['images']) && (count($data['setting']['images'])>0)){
						$dataInit = ['fullscreen', 'autoplay', 'stop_auto', 'navigation', 'pagination', 'loop', 'rtl'];
						...
						$content = '{{block class="MGS\Mpanel\Block\Widget\OwlCarousel" images="'.$images.'" links="'.$link.'" fullscreen="'.$data['setting']['fullscreen'].'" autoplay="'.$data['setting']['autoplay'].'" stop_auto="'.$data['setting']['stop_auto'].'" navigation="'.$data['setting']['navigation'].'" pagination="'.$data['setting']['pagination'].'" loop="'.$data['setting']['loop'].'" rtl="'.$data['setting']['rtl'].'" speed="'.$speed.'" effect="'.$data['setting']['effect'].'" template="widget/owl_slider.phtml"}}';
				...
				/* Deals Block */
				case "deals":
					$dataInit = ['use_slider', 'autoplay', 'stop_auto', 'navigation', 'pagination', 'use_tabs', 'loadmore','layout_deal'];
					$data = $this->reInitData($data, $dataInit);
					$categories = '';
					if(isset($data['setting']['category_id'])){
						$categories = implode(',',$data['setting']['category_id']);
				...
				case "special_product":
					$dataInit = ['product_name', 'product_price', 'product_rating', 'product_categories', 'product_description', 'product_addcart'];
					$data = $this->reInitData($data, $dataInit);
					
					$content = '{{block class="MGS\Mpanel\Block\Products\SpecialProduct" mgs_panel_title="'.$this->encodeHtml($data['setting']['title']).'" mgs_panel_note="'.$this->encodeHtml($data['setting']['additional_content']).'" product_id="'.$data['setting']['product_id'].'" ratio="'.$data['setting']['ratio'].'" product_name="'.$data['setting']['product_name'].'" product_price="'.$data['setting']['product_price'].'" product_rating="'.$data['setting']['product_rating'].'" product_categories="'.$data['setting']['product_categories'].'" product_description="'.$data['setting']['product_description'].'" product_addcart="'.$data['setting']['product_addcart'].'"';
				...
				/* Latest Posts Block */
				case "latest_posts":
					$dataInit = ['show_short_content', 'view_as', 'show_thumbnail', 'autoplay', 'stop_auto', 'navigation', 'pagination'];
					$data = $this->reInitData($data, $dataInit);
					$content = '{{widget type="MGS\Blog\Block\Widget\Latest" mgs_panel_title="'.$this->encodeHtml($data['setting']['title']).'" number_of_posts="'.$data['setting']['number_of_posts'].'" items="'.$data['setting']['items'].'" show_thumbnail="'.$data['setting']['show_thumbnail'].'" mgs_panel_note="'.$this->encodeHtml($data['setting']['additional_content']).'" template="widget/'.$data['setting']['template'].'"';
					...
					if($data['setting']['view_as']){
						$content .= ' view_as="owl_carousel" autoplay="'.$data['setting']['autoplay'].'" stop_auto="'.$data['setting']['stop_auto'].'" navigation="'.$data['setting']['navigation'].'" pagination="'.$data['setting']['pagination'].'"';
					}
				...
				case "facebook":
					...
					$data = $this->reInitData($data, $dataInit);
					$tabs = implode(',',$data['setting']['tabs']);
					$content = '{{block class="MGS\Social\Block\Panel\Widget" mgs_panel_title="'.$this->encodeHtml($data['setting']['title']).'" mgs_panel_note="'.$this->encodeHtml($data['setting']['additional_content']).'" page_url="'.$data['setting']['page_url'].'" width="'.$data['setting']['width'].'" height="'.$data['setting']['height'].'" tabs="'.$tabs.'" hide_cover="'.$data['setting']['hide_cover'].'" show_facepile="'.$data['setting']['show_facepile'].'" show_posts="'.$data['setting']['show_posts'].'" small_header="'.$data['setting']['small_header'].'" fit_inside="'.$data['setting']['fit_inside'].'" template="widget/fanbox.phtml"}}';
				...
				/* Twitter Feed Block */
				case "twitter":
					$content = '{{block class="MGS\Social\Block\Panel\Widget" mgs_panel_title="'.$this->encodeHtml($data['setting']['title']).'" mgs_panel_note="'.$this->encodeHtml($data['setting']['additional_content']).'" feed_count="'.$data['setting']['feed_count'].'" truncate="'.$data['setting']['truncate'].'" template="widget/twitter_feed.phtml"}}';
				...
				/* Instagram Block */
				case "instagram":
					$dataInit = ['link', 'like', 'comment'];
					$data = $this->reInitData($data, $dataInit);
					$content = '{{block class="MGS\Social\Block\Panel\Widget" mgs_panel_title="'.$this->encodeHtml($data['setting']['title']).'" mgs_panel_note="'.$this->encodeHtml($data['setting']['additional_content']).'" limit="'.$data['setting']['limit'].'" resolution="'.$data['setting']['resolution'].'" link="'.$data['setting']['link'].'" like="'.$data['setting']['like'].'" comment="'.$data['setting']['comment'].'" template="widget/instagram.phtml"}}';
