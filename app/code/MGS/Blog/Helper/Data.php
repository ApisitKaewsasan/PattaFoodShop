    ...
    {	
		...
		if($post->getThumbType() == "video"){
			if($post->getVideoThumbId() != ""){
				...
				$html .= '<iframe width="1024" height="768" src="'.$video_url.'" frameborder="0"></iframe>';
				$html .= '</div>';
			}
		}else {
			if($post->getThumbnailUrl() == ""){
				...
				if($store){
					$html .= '<a class="post-title-link" title="'.$post->getTitle().'" href="'.$post->getPostUrlWithNoCategory().'?___store='.$store.'">';
				}else{
					$html .= '<a class="post-title-link" title="'.$post->getTitle().'" href="'.$post->getPostUrlWithNoCategory().'">';
				}				
				$html .= '<img class="img-responsive" alt="'.$post->getTitle().'" src="'.$post->getThumbnailUrl().'"/>';
				$html .= '</a>';  
			}
		}
        return $html;
